...
$(function(){
	...
	{
		//alert(getParameter[1]);
		var params = getParameter[1].split("&");
		...
		for( i = 0; i < params.length; i++  )
		{
			...
			if( keyValue[0] == "page" )
			{
				//alert("page = " + keyValue[1]);
				nowPage = parseInt(keyValue[1]);
			}
			else if( keyValue[0] == "id_project" )
			{
				//alert("id_project = " + keyValue[1]);
				id_project = parseInt(keyValue[1]);
			}
		}
	...
	$("#header").load("./common_header?id_project=" + id_project);
	...
	function excelSelect(data){
		...
		$.ajax({
			...
			data:"table="+excelTable+"&params="+data+"&id_project="+id_project,
			dataType:"json",
			...
			error:function(request,status,error)
			{
				...
				// alert("안돼냐? " + request.responseText);
			}
		});
	}
				...
				$.ajax({
					...
					success: function(data){
						if(data["status"]=="success"){
							readExcelFile(data["folder"],data["file"], id_project);
						}else{
							Spinner("off");
	...
	function readExcelFile(folder,file,id_project){
		//alert("readExcelFile() : " + folder + " / " + file + " / " + id_project);
		var excelPlus = new ExcelPlus();
			...
			if(!passed){
				...
				if(dataList!=null && dataList!="")
				{
					...
					$.ajax({
						...
						data: "dataList="+dataList+"&table="+folder+"&id_project="+id_project,
